...
## Guía de Desarrollo Seguro para JAMR_4

**Creado:** 30 Oct 2025  
**Aplicable a:** Todos los fixes desde FIX-2 en adelante  
...
**Ejemplo FIX-2 (Persistencia):**
```
✅ Tocar:
  - gsmlte.h (agregar struct + funciones)
  - gsmlte.cpp (implementar funciones)
  - JAMR_4.ino (llamar load/save)

❌ NO tocar:
  - type_def.h (sin cambios en payload)
  - sensores.cpp/h (no relacionado)
  - cryptoaes.cpp/h (no relacionado)
  - sleepdev.cpp/h (no relacionado)
  - Lógica de watchdog (ya validada)
```
...
ModemPersistentState persistentState = {
  ...
  60000,   // avgConnectionTime - Razonable
  0.0,     // lastGPSLat - Inválido = buscar GPS
  0.0,     // lastGPSLon
  0        // lastGPSTime - Sin cache
};
...
ModemPersistentState persistentState = {
  ...
  -1,      // consecutiveFailures - ¡Inválido!
  0,       // avgConnectionTime - ¡Timeout cero!
  NULL,    // lastGPSLat - ¡Crash!
};
```
...
void loadPersistedState() {
  ...
  int rssi = modemPrefs.getInt("rssi", 15);  // Default si no existe
  
  // 🛡️ Validar rango
  if (rssi < 0 || rssi > 31) {
    rssi = 15;  // Default seguro
...
**Header global o por módulo:**
```cpp
// En gsmlte.h o config.h
#define ENABLE_PERSISTENCE true      // FIX-2
#define ENABLE_ADAPTIVE_TIMEOUT true  // FIX-3 (futuro)
#define ENABLE_BAND_LEARNING true     // FIX-4 (futuro)
```
...
void savePersistedState() {
    ...
    modemPrefs.end();
    logMessage(3, "💾 Estado guardado");
  #else
    logMessage(3, "💾 Persistencia deshabilitada");
  #endif
}

// En lógica de decisión
if (ENABLE_PERSISTENCE && persistentState.lastSuccessfulBand > 0) {
  // Lógica nueva
} else {
  // Lógica legacy (siempre funciona)
}
```
...
```cpp
void loadPersistedState() {
  logMessage(2, "💾 [PERSIST] Cargando estado desde NVS...");
  
  modemPrefs.begin("modem", true);
  persistentState.lastRSSI = modemPrefs.getInt("rssi", 15);
  modemPrefs.end();
  
  logMessage(2, "💾 [PERSIST] RSSI: " + String(persistentState.lastRSSI));
  logMessage(2, "💾 [PERSIST] Band: " + String(persistentState.lastSuccessfulBand));
  logMessage(2, "💾 [PERSIST] Fallos: " + String(persistentState.consecutiveFailures));
  
  if (persistentState.lastRSSI < 0 || persistentState.lastRSSI > 31) {
    logMessage(1, "⚠️ [PERSIST] RSSI inválido (" + String(persistentState.lastRSSI) + "), usando default");
    persistentState.lastRSSI = 15;
  }
}
```
...
```cpp
if (ENABLE_PERSISTENCE && persistentState.lastSuccessfulBand > 0) {
  logMessage(2, "📶 [PERSIST] Usando Band " + String(persistentState.lastSuccessfulBand) + " guardada");
  // ... lógica ...
} else {
  logMessage(2, "📶 [LEGACY] Búsqueda estándar de bandas");
  // ... lógica legacy ...
}
```

### Prefijos de contexto
- `[PERSIST]` - Operaciones de persistencia
- `[LEGACY]` - Código legacy (fallback)
- `[GPS]` - Operaciones GPS
- `[LTE]` - Operaciones LTE
- `[MODEM]` - Operaciones módem
...
if (!sendATCommand("+CBANDCFG=\"CAT-M\",2,4,5", "OK", timeout)) {
  logMessage(1, "⚠️ Fallo bandas");
  return false;  // CAMBIADO: agregamos return
}

// RIESGO: Si nuevo código falla, lógica original también falla
```

**✅ BIEN - Agregar lógica ANTES, preservar original:**
```cpp
// 🆕 FIX-2: Intentar banda persistida primero
if (ENABLE_PERSISTENCE && persistentState.lastSuccessfulBand > 0) {
  String bandCmd = "+CBANDCFG=\"CAT-M\"," + String(persistentState.lastSuccessfulBand);
  if (sendATCommand(bandCmd, "OK", timeout)) {
    logMessage(2, "✅ [PERSIST] Band exitosa");
    goto band_configured;  // Saltar lógica original
  } else {
    logMessage(1, "⚠️ [PERSIST] Band falló, usando búsqueda estándar");
  }
}
...
```
┌─────────────────────────┐
│   Lógica nueva (FIX-2)  │ ← Intenta primero
│   Si falla → continúa   │
├─────────────────────────┤
│  Lógica legacy (v4.1.0) │ ← Siempre funciona
│     Código original     │
└─────────────────────────┘
```
...
void testNVS() {
  ...
  persistentState.lastRSSI = 0;
  loadPersistedState();
  
  if (persistentState.lastRSSI == 9) {
...
### Baseline (v4.1.0 actual)
```
📊 BASELINE v4.1.0:
   Tiempo total ciclo: 58.3s
   GPS intentos: 2
   LTE tiempo: ~40s
   Watchdog resets: 0
   Transmisiones exitosas: 100%
   Consumo promedio: ~50mA activo
```
...
|---------|-------------|---------------------|
| **Tiempo total** | Debe ser ≤ baseline | No aumentar |
| **Watchdog resets** | Debe ser = 0 | CRÍTICO: cero resets |
| **GPS intentos** | Puede ser ≤ baseline | Mejora aceptable |
| **LTE tiempo** | Puede ser ≤ baseline | Mejora aceptable |
| **Éxito transmit** | Debe ser = 100% | Sin degradación |
| **Memoria libre** | Debe ser ≥ 80% baseline | Sin leaks |
| **Consumo** | Debe ser ≤ baseline | No aumentar |

### Formato de reporte
```markdown
## Comparación v4.1.0 vs v4.2.0

| Métrica | v4.1.0 | v4.2.0 | Δ | Status |
|---------|--------|--------|---|--------|
| Tiempo ciclo | 58.3s | 56.1s | -2.2s | ✅ MEJORA |
| Watchdog | 0 | 0 | 0 | ✅ OK |
| GPS intentos | 2 | 1 | -1 | ✅ MEJORA |
| LTE tiempo | 40s | 38s | -2s | ✅ MEJORA |
| Éxito TX | 100% | 100% | 0% | ✅ OK |
| RAM libre | 120KB | 115KB | -5KB | ✅ OK |
```
...
### Plan A: Feature flag
```cpp
// Opción más rápida (5 min)
#define ENABLE_PERSISTENCE false
platformio run -t upload
```

### Plan B: Volver a main
```bash
...
```cpp
// 🆕 FIX-2: Persistencia de estado entre reinicios
// Fecha: 30 Oct 2025
// Autor: [Nombre]
// Propósito: Guardar RSSI/Band/GPS en NVS para reutilizar después de reinicio
// Riesgo: BAJO - No modifica lógica existente
// Validado: [Fecha] en hardware
// Rollback: #define ENABLE_PERSISTENCE false
...
### Documentos requeridos por fix

**Durante implementación:**
1. `FIX-N_PLAN_EJECUCION.md` - Plan detallado
2. `FIX-N_LOG_PASO1.md` - Logs de cada paso
3. `FIX-N_LOG_PASO2.md`
4. `FIX-N_LOG_PASO3.md`

**Después de testing:**
5. `FIX-N_VALIDACION_HARDWARE.md` - Resultados de testing
6. `FIX-N_REPORTE_FINAL.md` - Conclusiones y métricas

**Actualizar:**
7. `SNAPSHOT_YYYYMMDD.md` - Estado actual del proyecto
8. `CHANGELOG.md` - Registro de cambios

### Commits descriptivos
```bash
# ✅ BIEN
git commit -m "feat(FIX-2): Implementar persistencia de estado en NVS

- Agregar struct ModemPersistentState en gsmlte.h
- Implementar load/savePersistedState() en gsmlte.cpp
- Integrar en setup() y antes de sleep en JAMR_4.ino
- Validado en hardware: 0 watchdog resets
- Tiempo ciclo: 58.3s → 56.1s (-3.8%)

Refs: FIX-2_PLAN_EJECUCION.md"

# ❌ MAL
git commit -m "fix: arreglos varios"
```
...
- ✅ **Branch dedicado** - Ningún impacto en main
- ✅ **Cambios mínimos** - Solo delays fragmentados
- ✅ **Validación hardware** - Detectó mejoras inesperadas
- ✅ **Documentación exhaustiva** - Fácil de revisar después

### Lo que mejorar
- 🔄 **Testing más gradual** - Saltamos directo a hardware
- 🔄 **Métricas baseline** - No teníamos medidas previas claras
- 🔄 **Feature flag** - No había, dificulta A/B testing

### Aplicado en FIX-2+
Todas las premisas de este documento incorporan lecciones de FIX-1.
...
- [ ] ✅ Documento FIX-N_LOG_PASOM.md creado
...
```markdown
# Validación FIX-N: [Nombre del Fix]

## Hardware
- Device ID: ___________
- Versión anterior: v4.X.X
- Versión nueva: v4.Y.Y
...
|---------|----------|-------|---|--------|
| Tiempo ciclo | __ | __ | __ | __ |
| Watchdog resets | 0 | __ | __ | __ |
| GPS intentos | __ | __ | __ | __ |
| LTE tiempo | __ | __ | __ | __ |
| Éxito TX | 100% | __ | __ | __ |
| RAM libre | __ | __ | __ | __ |
