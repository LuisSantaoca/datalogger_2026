...
// DESCRIPCIÓN: Sistema de lectura y gestión de sensores para monitoreo ambiental
// FUNCIONALIDADES:
//   - Lectura de sondas de suelo (RS485 Modbus)
//   - Sensores ambientales (AHT10/AHT20)
//   - Monitoreo de batería
//   - Sistema robusto de manejo de errores
//   - Logging estructurado y validación de datos
// =============================================================================
...
void setupSensores(sensordata_type* data) {
  ...
  Serial.print("Sonda: "); Serial.println(sensorStatus.sondaOK ? "✅ OK" : "❌ ERROR");
  Serial.print("AHT: "); Serial.println(sensorStatus.ahtOK ? "✅ OK" : "❌ ERROR");
  Serial.print("Batería: "); Serial.println(sensorStatus.bateriaOK ? "✅ OK" : "❌ ERROR");
  
  if (sensorStatus.erroresSonda > 0) {
    Serial.print("Errores de sonda: "); Serial.println(sensorStatus.erroresSonda);
  }
  if (sensorStatus.erroresAHT > 0) {
    Serial.print("Errores de AHT: "); Serial.println(sensorStatus.erroresAHT);
  }
  if (sensorStatus.erroresBateria > 0) {
    Serial.print("Errores de batería: "); Serial.println(sensorStatus.erroresBateria);
  }
  Serial.println("========================");
}
...
void read_dfrobot_sonda_ec() {
  ...
  if (readSondaRS485(request, 8, response, 12, "Sonda DFROBOT EC")) {
    ...
    if (response[3] == 0x00 && response[4] == 0x00 && 
        response[5] == 0x00 && response[6] == 0x00) {
...
void read_dfrobot_sonda_ecph() {
  ...
  if (readSondaRS485(request, 8, response, 12, "Sonda DFROBOT EC PH")) {
    // Validar que los datos no sean todo ceros
    if (response[3] == 0x00 && response[4] == 0x00 && 
        response[5] == 0x00 && response[6] == 0x00) {
    ...
    sensorStatus.sondaOK = true;
    sensorStatus.erroresSonda = 0;
    Serial.println("✅ Sonda DFROBOT EC PH - Datos procesados correctamente");
  } else {
    // En caso de error, limpiar variables y actualizar estado
    Serial.println("❌ Sonda DFROBOT EC PH - Error en comunicación, limpiando datos");
    H_temperatura_suelo = 0x00;
    L_temperatura_suelo = 0x00;
    H_humedad_suelo = 0x00;
    L_humedad_suelo = 0x00;
...
void readAht10() {
  ...
  if (temp < -4000 || temp > 8000 || hum < 0 || hum > 10000) {
    ...
    Serial.print("Temp: "); Serial.print(temp); Serial.println(" (debería estar entre -4000 y 8000)");
    Serial.print("Hum: "); Serial.print(hum); Serial.println(" (debería estar entre 0 y 10000)");
  }
