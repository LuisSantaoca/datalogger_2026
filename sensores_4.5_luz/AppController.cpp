...
#include "src/FeatureFlags.h"  // FIX-V1/V2
...
static const uint8_t DISCARD_SAMPLES = 20;
...
void AppInit(const AppConfig& cfg) {
  ...
  // ============ [FIX-V1 START] Imprimir flags activos al arranque ============
  printActiveFlags();
  // ============ [FIX-V1 END] ============
    ...
    case AppState::Cycle_SendLTE:
      {
        ...
        // ============ [FIX-V1 START] Log semanticamente correcto segun resultado ============
        bool lteResult = sendBufferOverLTE_AndMarkProcessed();
        if (lteResult) {
          Serial.println("[INFO][APP] Envio LTE exitoso, pasando a CompactBuffer");
        } else {
          Serial.println("[WARN][APP] Envio LTE fallo, pasando a CompactBuffer de todas formas");
        }
        // ============ [FIX-V1 END] ============
