...
bool initializeRTC() {
  pinMode(DEVICE_ENABLE_PIN_LOW, OUTPUT);
  pinMode(DEVICE_ENABLE_PIN_HIGH, OUTPUT);
  ...
  if (!rtc.isrunning()) {
    DEBUG_WARN(RTC, "RTC no está en ejecución, configurando hora...");
...
void enableDevice() {
  digitalWrite(DEVICE_ENABLE_PIN_LOW, LOW);
  digitalWrite(DEVICE_ENABLE_PIN_HIGH, HIGH);
}

void disableDevice() {
  digitalWrite(DEVICE_ENABLE_PIN_LOW, HIGH);
  digitalWrite(DEVICE_ENABLE_PIN_HIGH, LOW);
}
